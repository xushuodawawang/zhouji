...
## V2.5 多彩统计与本地歌单（2026-09-10）

- 课表扩展为森林绿、橄榄绿、叶绿、柔黄、奶油色、蓝灰、雾粉、湖蓝、薰衣草紫、珊瑚色、琥珀色和青绿色；同名任务继续保持同色。
- 专注饼图按当前数据顺序分配高区分度颜色，相邻项目不再挤在相似绿色中。
- “专注背景音乐”支持一次选择多首本地音频，保存歌单并按顺序循环播放；暂停、继续、完成停止的行为与单曲一致。
- 单曲和本地歌单可以随时互相切换，歌单配置在重启应用后仍保留。
- 网易云音乐、QQ 音乐的在线歌单需要各自平台的正式授权与合作凭证；当前版本支持导入 Android 文件选择器能够访问的 MP3、FLAC、AAC 等普通音频文件。
- 数据库平滑升级为 schema V7，原有计划、记录和设置全部保留。

验证：Flutter analyze 无问题，59 项自动测试全部通过，覆盖 V1－V6 数据升级、歌单序列化、连续播放调用以及手机尺寸布局。

![V2.5 多彩课表](docs/screenshots/v2.5-plan.png)
![V2.5 多彩统计图](docs/screenshots/v2.5-statistics.png)
![V2.5 本地歌单](docs/screenshots/v2.5-focus-tools.png)

## V2.4 专注锁定、音乐与柔和配色（2026-09-10）

- 锁定任务的图标移到卡片右上角并加上独立浅色底，不再与居中的任务名称重叠。
- 课表采用芽绿、淡叶绿、晴空蓝、柔日黄、雾粉和薰衣草等柔和色板；已完成或未完成任务仍保留可辨认的原任务颜色。
- 统计背景参考浅绿色数据面板，专注饼图使用层次清楚的绿色色阶，并以圆角间隔区分相邻项目。
- 新增“专注时锁定手机”：开始专注时调用 Android 系统屏幕固定，结束、提前结束或重置后自动解除。
- 本地背景音乐支持选择、更换和试听；专注进行中可以直接暂停或继续音乐。
- 新增默认开启的番茄完成铃声，整段专注完成时播放内置轻柔双音铃声；后台通知也明确开启声音和振动。

验证：Flutter analyze 无问题，56 项自动测试全部通过，覆盖 V1－V5 数据升级、专注锁定生命周期、音乐播放控制、完成铃声和锁图标布局。

![V2.4 柔和课表配色](docs/screenshots/v2.4-plan.png)
![V2.4 绿色统计图](docs/screenshots/v2.4-statistics.png)
![V2.4 专注锁定与音乐](docs/screenshots/v2.4-focus-tools.png)

## V2.3 专注回填与紧凑计划表（2026-09-10）

- 从“常用专注”开始数学、408 等临时任务时，会立即在当前时间绘制对应计划；同一时段已有其他计划时，将其替换为实际正在进行的任务。直接点击主计时器开始，仍会优先采用此刻的原计划。
- 移除任务编辑器和应用设置中的分类操作。专注统计改为直接按任务名称汇总，省去逐项选择分类的步骤。
- 同名任务使用稳定的自动颜色，忽略名称前后空格、连续空格和英文大小写；升级后会在自动配色开启时统一旧任务颜色。
- 修复低倍率缩放时任务卡被强制撑高导致的错位、分栏和大小不一。卡片严格按时间比例排列，文字按卡片高度自动缩放并水平、垂直居中；时间标签过密时自动降低显示频率。
- 计划页将品牌、周／月／日切换、日期导航、视图、缩放和新建操作压缩为两行，让时间表获得更多可视空间。
- 空白时间段支持长按后向上或向下滑动选择范围，并加入轻触反馈；移动按钮改为更小的居中控制块，紧凑卡片会自动隐藏拥挤的控制图标。
- 统计页从青蓝色调整为绿色主题，专注环图采用按任务名称稳定分配的绿色色阶。

验证：Flutter analyze 无问题，54 项自动测试全部通过，覆盖专注覆盖计划、同名配色、最低缩放对齐、分类入口移除及主要页面手机尺寸渲染。

![紧凑计划表](docs/screenshots/v2.3-plan.png)
![绿色专注统计](docs/screenshots/v2.3-statistics.png)

## V2.2.1 任务卡可读性（2026-09-08）

- 长任务的标题改为在卡片可用空间内垂直居中，起止时间继续固定在底部。
- “计划 → 右上角设置”新增任务卡透明度，可在 25%–100% 间调节；透明度只作用于背景，文字和操作按钮保持清晰。
...
## V2.2 考研专注与全天计划（2026-09-08）

- 根据用户提供的番茄 ToDo 截图调整交互。专注页新增可重复使用的任务卡片，例如“数学 60 分钟”，支持新建、编辑、删除和正向计时。卡片采用本地渐变背景。
- 计划编辑器可单独设置每项任务的番茄时长。直接开始专注会默认采用此刻正在进行的未完成计划，包括前一天跨午夜延续的任务；没有当前计划时使用自由专注。
- 开启“完成专注后完成计划”，倒计时结束或手动完成会同步计划状态；提前结束只保存未完成专注记录。常用卡片优先匹配当前同名计划，其次匹配今天的同名未完成计划。
- 正向计时从零开始，支持暂停、继续、重启恢复；结束后计入专注时长，不计为完整番茄。
- 支持选择手机本地音频，专注时循环播放，暂停和结束时停止。音频通过 Android 文件选择器授权，异步准备播放。
- 时间轴默认 **00:00–24:00**，全局时间选择器使用 24 小时制。“计划”右上角可设置开始与结束时间，例如 06:00–次日 06:00。跨天任务显示“次日”，按所选计划日保存；时间轴自动展开以容纳已有任务，并检查相邻日期的绝对时间冲突。
- 压缩计划页标题、日期和操作栏；缩放最小值降至每小时 12dp，使全天适配能容纳 24 小时。
- 默认自动配色：优先采用分类颜色，否则根据任务名稳定分配颜色；设置中可切换回手动选色。
- 活动记录可以标为未完成；未完成记录保留内容，但不计入实际完成时长。
- 统计页采用青色渐变背景和白色卡片，提供累计专注、当日专注、日／周／月／自定义时长分布、专注记录和本月 24 小时时段分布。日均按首次专注至今天的自然日计算，跨日专注记录归属开始日；时段图按实际时长在起止区间内分摊。
- 数据库升级为 schema v4，保留旧任务、记录和设置，增加常用专注表、每任务时长、未完成记录状态、计时总时长及标题快照。

![专注任务卡片](docs/screenshots/v2.2-focus.png)
![专注统计](docs/screenshots/v2.2-statistics.png)

截图由真实 Flutter 页面与演示数据渲染。安装包可以覆盖同签名旧版；手机本地音乐、通知和厂商后台限制仍需在实际设备上验证。

验证：50 项测试通过，覆盖 V1/V2/V3 升级、跨天冲突、常用卡片操作、60 分钟恢复、自动计划关联与完成同步、正向计时、窄屏布局及秒级局部刷新。

适合后续增加的考研功能：各科每周时长目标、错题间隔复习、未完成任务顺延、考试倒计时，以及优先保障学习记录安全的本地备份与恢复。
...
- 详细模式默认 `00:00–24:00`；支持自定义时间范围、跨天显示、加减按钮、双指捏合和“适配整天”缩放。
- 缩放范围为每小时 12–120dp，缩放时保持当前查看时间居中，不会改变任务的真实起止时间。
...
- 定时任务发生重叠时阻止保存，包括相邻日期的跨天冲突，并提示冲突任务名称；全天任务不参与冲突。
- 月视图显示任务摘要，长按日期创建全天任务，并支持月目标。
- 日计划汇总全天/定时任务、实际记录摘要和每日总结。

### 专注

- 根据当前时间自动采用正在进行的未完成计划。
...
从 V1、V2、V3 升级时使用 Drift `onUpgrade` 原位增加字段和创建新表，不会删除原有任务、记录或总结。V4 新增 `FocusPresets` 和专注、记录、时间轴设置字段。默认分类和默认设置使用幂等插入。`database_migration_test.dart` 会实际创建历史数据库并验证升级后的旧数据。
